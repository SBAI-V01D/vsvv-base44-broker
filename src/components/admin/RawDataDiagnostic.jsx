...
export default function RawDataDiagnostic({ showOnly = false }) {
  ...
  const { data: customerCount = 0 } = useQuery({
    ...
    queryFn: async () => {
      const all = await vsvv.entities.Customer.list('', 1000)
      return all?.length || 0
    },
  })
  ...
  const { data: contractCount = 0 } = useQuery({
    ...
    queryFn: async () => {
      const all = await vsvv.entities.Contract.list('', 1000)
      return all?.length || 0
    },
  })
  ...
  const { data: applicationCount = 0 } = useQuery({
    ...
    queryFn: async () => {
      const all = await vsvv.entities.Application.list('', 1000)
      return all?.length || 0
    },
  })
  ...
  const { data: documentCount = 0 } = useQuery({
    ...
    queryFn: async () => {
      const all = await vsvv.entities.Document.list('', 1000)
      return all?.length || 0
    },
  })
