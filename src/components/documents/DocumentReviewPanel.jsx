...
export default function DocumentReviewPanel({ document, onClose, onSaved }) {
  ...
  const { data: customers = [], isSuccess: customersLoaded } = useQuery({
    queryKey: ['customers'],
    queryFn: () => vsvv.entities.Customer.list(null, 1000),
    staleTime: 60_000,
  })
  ...
  const runExtract = async () => {
    ...
    let res
    try {
      res = await vsvv.functions.invoke('extractApplicationData', {
  ...
  const doSaveAutoMode = async (normalized, flat, produkteData, candidates, topScore) => {
    ...
    if (topScore >= 80 && candidates.length > 0) {
      ...
      try {
        const syncResult = await vsvv.functions.invoke('syncCustomerFromApplication', {
        ...
        const newC = await vsvv.entities.Customer.create({
          first_name: flat.first_name || 'Unbekannt',
          last_name:  flat.last_name || 'Unbekannt',
    ...
    try {
      ...
      newApp = await vsvv.entities.Application.create({
        customer_id: cid,
                      ...
                      onSelect={async (c) => {
                        // ─── HARD PERSISTENCE FIX: document.customer_id = SINGLE SOURCE OF TRUTH ───
                          try {
                            ...
                            const reloadedDoc = await vsvv.entities.Document.get(document.id)
