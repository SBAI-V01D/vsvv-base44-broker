...
export default function SmartDocumentSuggestions({ document, insights, onSuccess, onEdit }) {
  ...
  const createCustomerMut = useMutation({
    mutationFn: async (data) => avasys.entities.Customer.create(data),
  })
  const createFamilyMut = useMutation({
    mutationFn: async (data) => avasys.functions.invoke('createFamilyMember', data),
  })
  const createContractMut = useMutation({
    mutationFn: async (data) => avasys.entities.Contract.create(data),
  })
  const updateDocMut = useMutation({
    mutationFn: async ({ id, updates }) => avasys.entities.Document.update(id, updates),
  })
  ...
  const finalize = async (customerId, primaryCustomerId, orgId, advisorId) => {
    ...
    if (!resolvedOrgId) {
      const orgs = await avasys.entities.Organization.list('-created_date', 1)
  ...
  const handleSaveFamilyMember = async () => {
    ...
    if (!primary?.organization_id) {
      const results = await avasys.entities.Customer.filter({ id: primaryId })
