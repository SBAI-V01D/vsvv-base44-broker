...
export default function SmartDocumentSuggestions({ document, insights, onSuccess, onEdit }) {
  ...
  const createCustomerMut = useMutation({
    mutationFn: async (data) => vsvv.entities.Customer.create(data),
  })
  const createFamilyMut = useMutation({
    mutationFn: async (data) => vsvv.functions.invoke('createFamilyMember', data),
  })
  const createContractMut = useMutation({
    mutationFn: async (data) => vsvv.entities.Contract.create(data),
  })
  const updateDocMut = useMutation({
    mutationFn: async ({ id, updates }) => vsvv.entities.Document.update(id, updates),
  })
  ...
  const finalize = async (customerId, primaryCustomerId, orgId, advisorId) => {
    ...
    if (!resolvedOrgId) {
      const orgs = await vsvv.entities.Organization.list('-created_date', 1)
  ...
  const handleSaveFamilyMember = async () => {
    ...
    if (!primary?.organization_id) {
      const results = await vsvv.entities.Customer.filter({ id: primaryId })
