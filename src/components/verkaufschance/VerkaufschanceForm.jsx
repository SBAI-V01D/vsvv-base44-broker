...
export default function VerkaufschanceForm({ verkaufschance, customer, onSave, onCancel, saving, lead }) {
  ...
  useEffect(() => {
    if (lead && !selectedCustomer) {
      const searchFromLead = async () => {
        setIsSearching(true)
        try {
          const customers = await vsvv.entities.Customer.list()
  ...
  useEffect(() => {
    ...
    if (linkedContractId && !verkaufschance && !selectedCustomer) {
      const loadFromContract = async () => {
        setIsSearching(true)
        try {
          const contract = await vsvv.entities.Contract.get(linkedContractId)
          if (contract && customerIdFromUrl) {
            const customer = await vsvv.entities.Customer.get(customerIdFromUrl)
            if (customer) {
  ...
  useEffect(() => {
    ...
    const debounce = setTimeout(async () => {
      ...
      try {
        const contracts = await vsvv.entities.Contract.filter({ customer_id: selectedCustomer.id })
