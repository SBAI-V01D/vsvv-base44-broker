...
export default function DossierAiUpload({ dossierId, personName, onEntryAdded, onClose, knownPersons = [], defaultGruppe: propDefaultGruppe = 'aktuelle_loesung' }) {
  ...
  const analyzeMutation = useMutation({
    mutationFn: async (uploadedFile) => {
      const { file_url } = await avaai.integrations.Core.UploadFile({ file: uploadedFile });
      return avaai.integrations.Core.InvokeLLM({
  ...
  const saveMutation = useMutation({
    mutationFn: async (products) => {
      ...
      for (const p of products) {
        ...
        const existingInSameGroup = await avaai.entities.ComparisonEntry.filter(filterQuery).then(r => {
        ...
        if (existingInSameGroup) {
          ...
          const entry = await avaai.entities.ComparisonEntry.update(existingInSameGroup.id, {
          ...
          const entry = await avaai.entities.ComparisonEntry.create({
            dossier_id:        dossierId,
            person_name:       personNameFinal,
            section:           sectionFinal,
            gruppe:            targetGruppe,
