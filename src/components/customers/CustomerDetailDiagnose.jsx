...
export function useCustomerDetailDiagnose(customerId) {
  ...
  useEffect(() => {
    ...
    const observer = new PerformanceObserver((list) => {
      list.getEntries().forEach((entry) => {
        if (entry.name.includes('api/avasys') || entry.name.includes('entities')) {
  ...
  const checkDataFlow = async () => {
    ...
    try {
      ...
      const customer = await avasys.entities.Customer.filter({ id: customerId }, null, 1)
      ...
      const contracts = await avasys.entities.Contract.filter({ customer_id: customerId, archived: false })
      ...
      if (customer[0]?.is_family_member && customer[0]?.primary_customer_id) {
        ...
        const primary = await avasys.entities.Customer.filter({ id: customer[0].primary_customer_id }, null, 1)
      ...
      const advisors = await avasys.entities.Advisor.list()
