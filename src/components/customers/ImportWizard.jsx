...
export default function ImportWizard({ open, onOpenChange, onSuccess }) {
  ...
  const parseCSV = async (file) => {
    ...
    try {
      ...
      try {
        existingCustomers = await vsvv.entities.Customer.list('', 1000) || []
      } catch (err) {
  ...
  const handleImport = async () => {
    ...
    try {
      ...
      // Upload file
      const uploadRes = await vsvv.integrations.Core.UploadFile({ file });
      ...
      const importPromise = vsvv.functions.invoke('importEntityData', {
