...
export default function RenewalsSection() {
  ...
  const { data: offers = [] } = useQuery({
    ...
    queryFn: async () => {
      const all = await avasys.entities.Contract.list('-end_date')
  ...
  const prepareOfferMutation = useMutation({
    mutationFn: (contractId) =>
      avasys.functions.invoke('prepareRenewalOffer', { contract_id: contractId }),
    onSuccess: () => {
  ...
  const sendOfferMutation = useMutation({
    mutationFn: (contractId) =>
      avasys.functions.invoke('sendRenewalOffer', { contract_id: contractId }),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['contracts', 'renewal-offers'] })
  ...
  const acceptOfferMutation = useMutation({
    mutationFn: (contractId) =>
      avasys.functions.invoke('acceptRenewalOffer', { contract_id: contractId }),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['contracts', 'renewal-offers'] })
      setSelectedPolicy(null)
    },
