...
function RequestRow({ request }) {
  ...
  const updateStatus = useMutation({
    mutationFn: (newStatus) => avaai.entities.Interaction.update(request.id, { request_status: newStatus }),
...
export default function CustomerRequestsWidget() {
  ...
  const { data: interactions = [], isLoading } = useQuery({
    ...
    queryFn: () => avaai.entities.Interaction.filter({ is_customer_request: true }, '-created_date', 50),
  });
