...
export default function VergleichsAnalysenListe() {
  ...
  const { data: analysen = [], isLoading } = useQuery({
    ...
    queryFn: () => avaai.entities.VergleichsAnalyse.list('-created_date', 200),
    staleTime: 30 * 1000,
  });
  ...
  const handleDelete = async (analyse) => {
    ...
    try {
      await avaai.entities.VergleichsAnalyse.delete(analyse.id);
  ...
  const handleSaveWahl = async (analyse) => {
    await avaai.entities.VergleichsAnalyse.update(analyse.id, {
  ...
  const handleToggleAbgeschlossen = async (analyse) => {
    ...
    try {
      await avaai.entities.VergleichsAnalyse.update(analyse.id, { status: newStatus });
      queryClient.invalidateQueries({ queryKey: ['vergleichs-analysen'] });
    } finally {
      setTogglingId(null);
    }
