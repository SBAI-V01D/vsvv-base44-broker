...
export default function Tasks() {
  ...
  const { data: tasks = [], isLoading } = useQuery({
    queryKey: ['tasks'],
    queryFn: () => vsvv.entities.Task.list('-created_date', 200),
  });

  // Manuelle Datumsprüfung — Verträge mit Platzhalter-Datum
  const { data: reviewContracts = [] } = useQuery({
    queryKey: ['tasks_date_review'],
    queryFn: () => vsvv.entities.Contract.filter({ archived: false }, '-updated_date', 500),
    staleTime: 5 * 60 * 1000,
  });
  ...
  const updateMutation = useMutation({
    mutationFn: ({ id, data }) => vsvv.entities.Task.update(id, data),
    onSuccess: (updated) => {
  ...
  useEffect(() => {
    const unsubscribe = vsvv.entities.Task.subscribe((event) => {
      queryClient.setQueryData(['tasks'], (old = []) => {
