...
export default function CommissionsAndCourtage() {
  ...
  const { data: organizations = [] } = useQuery({
    ...
    queryFn: () => avaai.entities.Organization.filter({ status: 'active' }),
    staleTime: 60_000,
  })
  const { data: brokers = [] } = useQuery({
    queryKey: ['brokers'],
    queryFn: () => avaai.entities.Broker.list(),
    staleTime: 60_000,
  })
  const { data: customers = [] } = useQuery({
    queryKey: ['customers-for-commission'],
    queryFn: () => avaai.entities.Customer.list(null, 2000),
    staleTime: 60_000,
  })
  const { data: contracts = [] } = useQuery({
    queryKey: ['contracts-for-commission'],
    queryFn: () => avaai.entities.Contract.filter({ archived: false }, null, 5000),
    staleTime: 60_000,
  })
  const { data: auditLogs = [] } = useQuery({
    queryKey: ['auditLogs-commission'],
    queryFn: () => avaai.entities.AuditLog.filter({ entity_type: 'commission' }, '-changed_at', 500),
  ...
  const writeAuditLog = useCallback(async (entityId, action, summary, oldValues, newValues) => {
    const user = await avaai.auth.me().catch(() => null)
    avaai.entities.AuditLog.create({
  ...
  const createMutation = useMutation({
    mutationFn: async (data) => {
      ...
      return avaai.entities.CommissionEntry.create(data.is_storno ? data : calcCommissionFields(data));
    },
    onSuccess: (created) => {
  ...
  const updateMutation = useMutation({
    mutationFn: ({ id, data, oldData }) =>
      avaai.entities.CommissionEntry.update(id, data.is_storno ? data : calcCommissionFields(data)).then(r => ({ result: r, oldData })),
  ...
  const archiveMutation = useMutation({
    mutationFn: ({ id, entry }) =>
      avaai.entities.CommissionEntry.update(id, { archived: true, archived_at: new Date().toISOString() })
  ...
  const statusChangeMutation = useMutation({
    mutationFn: ({ id, newStatus, entry, type }) => {
      ...
      return avaai.entities.CommissionEntry.update(id, updates).then(r => ({ result: r, entry, newStatus, type }))
    },
