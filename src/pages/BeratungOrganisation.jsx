...
export default function BeratungOrganisation() {
  ...
  const { data: orgs = [] } = useQuery({ queryKey: ['organizations'], queryFn: () => avaai.entities.Organization.list('-created_date') })
  const { data: advisors = [] } = useQuery({ queryKey: ['advisors'], queryFn: () => avaai.entities.Advisor.list('-created_date') })

  const createOrg = useMutation({ mutationFn: d => avaai.entities.Organization.create(d), onSuccess: () => { qc.invalidateQueries(['organizations']); setShowOrgForm(false) } })
  const updateOrg = useMutation({ mutationFn: ({ id, data }) => avaai.entities.Organization.update(id, data), onSuccess: () => { qc.invalidateQueries(['organizations']); setShowOrgForm(false); setEditingOrg(null) } })
  const deleteOrg = useMutation({ mutationFn: id => avaai.entities.Organization.delete(id), onSuccess: () => qc.invalidateQueries(['organizations']) })

  const createAdvisor = useMutation({ mutationFn: d => avaai.entities.Advisor.create(d), onSuccess: () => { qc.invalidateQueries(['advisors']); setShowAdvisorForm(false) } })
  const updateAdvisor = useMutation({ mutationFn: ({ id, data }) => avaai.entities.Advisor.update(id, data), onSuccess: () => { qc.invalidateQueries(['advisors']); setShowAdvisorForm(false); setEditingAdvisor(null) } })
  const deleteAdvisor = useMutation({ mutationFn: id => avaai.entities.Advisor.delete(id), onSuccess: () => qc.invalidateQueries(['advisors']) })
