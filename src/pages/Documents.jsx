...
export default function Documents() {
  ...
  const { data: documents = [], isLoading } = useQuery({
    queryKey: ['documents'],
    queryFn: () => vsvv.entities.Document.list('-created_date'),
  })
  ...
  const deleteBulkMutation = useMutation({
    mutationFn: async () => {
      ...
      for (const doc of toDelete) {
        await vsvv.entities.Document.delete(doc.id)
      }
  ...
  const handleRequeue = async (doc) => {
    await vsvv.entities.AutomationQueue.create({
    ...
    try {
      ...
      if (res.data?.success) {
        ...
        if (res.data.detected_doc_type && res.data.detected_doc_type !== doc.doc_type) {
          vsvv.entities.Document.update(doc.id, {
  ...
  const handleFixClassificationStatus = async () => {
    ...
    for (const doc of docsToFix) {
      vsvv.entities.Document.update(doc.id, { classification_status: 'klassifiziert' }).catch(() => {
