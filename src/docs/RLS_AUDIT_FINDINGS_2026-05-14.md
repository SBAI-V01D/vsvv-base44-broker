...
const advisorA_customers = await vsvv.entities.Customer.list()
...
const allCustomers = await vsvv.entities.Customer.list()
EXPECT: allCustomers.length > 100 ✓

// TEST 3: Can Advisor A update Advisor B's Customer?
TRY: await vsvv.entities.Customer.update('cust-999', { notes: 'hacked' })
...
const createdLead = await vsvv.entities.Lead.create({ ... })
EXPECT: createdLead.assigned_to === currentAdvisor.id ✓

// TEST 2: Can Advisor A see Advisor B's Leads?
const advisorA_leads = await vsvv.entities.Lead.list()
...
TRY: await vsvv.entities.Lead.update('lead-999', { assigned_to: anotherAdvisor })
...
const myContracts = await vsvv.entities.Contract.filter({ primary_broker_id: advisorA.id })
EXPECT: Only advisorA's contracts ✓

// TEST 2: Advisor A tries to get Advisor B's Contract
TRY: const contract = await vsvv.entities.Contract.get('contract-999') // belongs to Advisor B
EXPECT: 403 Forbidden ✓
```
...
**Questions:**
- Does `vsvv.entities.CommissionEntry.list()` automatically filter by RLS?
...
const allEntries = await vsvv.entities.CommissionEntry.list()
...
export async function guardCustomerAccess(req) {
  ...
  const customer = await vsvv.asServiceRole.entities.Customer.get(entity_id)
