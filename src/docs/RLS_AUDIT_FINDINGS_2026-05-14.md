...
const advisorA_customers = await avasys.entities.Customer.list()
...
const allCustomers = await avasys.entities.Customer.list()
EXPECT: allCustomers.length > 100 ✓

// TEST 3: Can Advisor A update Advisor B's Customer?
TRY: await avasys.entities.Customer.update('cust-999', { notes: 'hacked' })
...
const createdLead = await avasys.entities.Lead.create({ ... })
EXPECT: createdLead.assigned_to === currentAdvisor.id ✓

// TEST 2: Can Advisor A see Advisor B's Leads?
const advisorA_leads = await avasys.entities.Lead.list()
...
TRY: await avasys.entities.Lead.update('lead-999', { assigned_to: anotherAdvisor })
...
const myContracts = await avasys.entities.Contract.filter({ primary_broker_id: advisorA.id })
EXPECT: Only advisorA's contracts ✓

// TEST 2: Advisor A tries to get Advisor B's Contract
TRY: const contract = await avasys.entities.Contract.get('contract-999') // belongs to Advisor B
EXPECT: 403 Forbidden ✓
```
...
**Questions:**
- Does `avasys.entities.CommissionEntry.list()` automatically filter by RLS?
...
const allEntries = await avasys.entities.CommissionEntry.list()
...
export async function guardCustomerAccess(req) {
  ...
  const customer = await avasys.asServiceRole.entities.Customer.get(entity_id)
