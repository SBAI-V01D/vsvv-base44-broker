...
**Aufruf:**
```javascript
const result = await avaai.functions.invoke('generateCustomerNumber', {});
...
// Keine Duplikate
const existing = await avaai.entities.Customer.filter({
...
await avaai.functions.invoke('migrateCustomerNumbers', {});
```
